"""
User Tower implementation for the music recommender system.

This module defines the neural network architecture for processing user features.
"""
...
class UserTower(nn.Module):
    """
    User tower of the two-tower recommender architecture.
    
    This component processes user features (demographics, listening behavior, 
    audio preferences) and outputs a user embedding vector.
    """
    ...
    def __init__(self, config: Dict, categorical_mappings: Optional[Dict] = None):
        ...
        for feature_name, embedding_config in config['user_tower']['embeddings'].items():
    ...
    def _init_dense_layers(self, input_size):
        ...
        for layer_config in self.config['user_tower']['layers']:
            # Add dense layer
        ...
        logger.info(f"Initialized UserTower with input size {self.input_size} and {len(self.layers)} layers")
    ...
    def _calculate_input_size(self, features):
        ...
        num_numerical = 0
        for feature_name in self.config['features']['user_features']['numerical']:
            if feature_name in features:
                num_numerical += 1
        
        # Count embedded dimensions from available categorical features
        embedded_dims = 0
        for feature_name, config in self.config['user_tower']['embeddings'].items():
            feature_id = f"{feature_name}_id"
            if feature_id in features:
                embedded_dims += config['embedding_dim']
        
        total_size = num_numerical + embedded_dims
        logger.info(f"Dynamic UserTower input size: {total_size} (numerical: {num_numerical}, embeddings: {embedded_dims})")
        
        if total_size == 0:
            logger.warning("No user features are available! Using a default input size")
    ...
    def forward(self, user_features: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for feature_name in self.config['features']['user_features']['numerical']:
            if feature_name in user_features:
                numerical_features.append(user_features[feature_name])
        
        # Process categorical features through embeddings
        embedded_features = []
        for feature_name in self.config['features']['user_features']['categorical']:
            if f"{feature_name}_id" in user_features and feature_name in self.embedding_layers:
                # Get feature tensor
                feature_tensor = user_features[f"{feature_name}_id"]
        ...
        if len(combined_features) > 1:
            ...
            logger.warning("No user features available for forward pass")
            batch_size = next(iter(user_features.values())).shape[0]
            x = torch.zeros((batch_size, 1), device=next(iter(user_features.values())).device)
