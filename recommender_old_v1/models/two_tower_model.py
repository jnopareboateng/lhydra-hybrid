...
class TwoTowerHybridModel(nn.Module):
    ...
    def forward(self, user_features, item_features):
        ...
        if not hasattr(self, '_debug_logged') or not self._debug_logged:
            ...
            print("\n" + "="*80)
            print("MODEL INPUT DEBUG INFORMATION")
            print("="*80)
            print(f"User features type: {type(user_features)}")
            if isinstance(user_features, dict):
                print(f"User features keys: {list(user_features.keys())}")
                for k, v in user_features.items():
                    if isinstance(v, torch.Tensor):
                        print(f"  - {k}: shape={v.shape}, dtype={v.dtype}")
            elif isinstance(user_features, torch.Tensor):
                print(f"User features tensor shape: {user_features.shape}, dtype: {user_features.dtype}")
            
            print(f"\nItem features type: {type(item_features)}")
            if isinstance(item_features, dict):
                print(f"Item features keys: {list(item_features.keys())}")
                for k, v in item_features.items():
                    if isinstance(v, torch.Tensor):
                        print(f"  - {k}: shape={v.shape}, dtype={v.dtype}")
            elif isinstance(item_features, torch.Tensor):
                print(f"Item features tensor shape: {item_features.shape}, dtype: {item_features.dtype}")
            print("="*80 + "\n")
        ...
        if not hasattr(self, '_embedding_logged') or not self._embedding_logged:
            self._embedding_logged = True
            print("\n" + "="*80)
            print("MODEL EMBEDDING DEBUG INFORMATION")
            print("="*80)
            print(f"User embedding shape: {user_embedding.shape}, dtype: {user_embedding.dtype}")
            print(f"Item embedding shape: {item_embedding.shape}, dtype: {item_embedding.dtype}")
            print("="*80 + "\n")
        ...
        if not hasattr(self, '_output_logged') or not self._output_logged:
            ...
            print("\n" + "="*80)
            print("MODEL OUTPUT DEBUG INFORMATION")
            print("="*80)
            print(f"Combined embeddings shape: {combined.shape}, dtype: {combined.dtype}")
            print(f"Prediction shape: {prediction.shape}, dtype: {prediction.dtype}")
            print(f"Prediction sample: {prediction[:5].detach().cpu().numpy()}")
            print("="*80 + "\n")
