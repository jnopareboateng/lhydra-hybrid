...
class HybridModelTrainer:
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        # Log training data information at the beginning of the first epoch
        if epoch == 1:
            logger.info("\n" + "="*80)
            logger.info("TRAINING DATA INFORMATION")
            logger.info("="*80)
            ...
            logger.info(f"User features shape: {user_features.shape}, dtype: {user_features.dtype}")
            # Sample a few values
            if user_features.shape[0] > 0:
                logger.info(f"User features first row: {user_features[0][:10].cpu().numpy()} (showing first 10 values)")
            
            # Log item features
            item_features = batch['item_features']
            logger.info(f"Item features shape: {item_features.shape}, dtype: {item_features.dtype}")
            # Sample a few values
            if item_features.shape[0] > 0:
                logger.info(f"Item features first row: {item_features[0][:10].cpu().numpy()} (showing first 10 values)")
            
            # Log targets
            targets = batch['target']
            logger.info(f"Targets shape: {targets.shape}, dtype: {targets.dtype}")
            logger.info(f"Targets sample: {targets[:10].cpu().numpy()}")
            
            # Log user_ids, item_ids if available
            if 'user_id' in batch:
                logger.info(f"User IDs sample: {batch['user_id'][:10]}")
            if 'item_id' in batch:
                logger.info(f"Item IDs sample: {batch['item_id'][:10]}")
            
            logger.info("="*80)
            logger.info("END OF TRAINING DATA INFORMATION")
            logger.info("="*80 + "\n")
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            # For the first batch of the first epoch, log detailed model input and output
            if epoch == 1 and batch_idx == 0:
                logger.info("\n" + "="*80)
                logger.info("FIRST BATCH MODEL I/O INFORMATION")
                logger.info("="*80)
                logger.info(f"Model input - User features shape: {user_features.shape}")
                logger.info(f"Model input - Item features shape: {item_features.shape}")
                logger.info(f"Model output shape: {outputs.shape}")
                logger.info(f"Model output sample (first 5 values): {outputs[:5].detach().cpu().numpy()}")
                logger.info(f"Target values (first 5): {targets[:5].cpu().numpy()}")
                logger.info("="*80 + "\n")
