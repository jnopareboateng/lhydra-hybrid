...
class HybridRecommenderDataset(Dataset):
    ...
    def __init__(self, interactions_df, user_features_df, item_features_df, target_col='engagement'):
        ...
        logger.info("\n" + "="*80)
        logger.info("DATASET INITIALIZATION - DETAILED INFORMATION")
        logger.info("="*80)
        
        logger.info(f"Interactions DataFrame shape: {self.interactions.shape}")
        logger.info(f"Interactions DataFrame columns: {list(self.interactions.columns)}")
        if not self.interactions.empty:
            sample_interaction = self.interactions.iloc[0]
            logger.info(f"Sample interaction: User ID={sample_interaction.get('user_id', 'N/A')}, "
                       f"Track ID={sample_interaction.get('track_id', 'N/A')}, "
                       f"Target={sample_interaction.get(self.target_col, 'N/A')}")
        
        logger.info(f"\nUser Features DataFrame shape: {self.user_features.shape}")
        logger.info(f"User Features DataFrame columns: {list(self.user_features.columns)}")
        logger.info(f"User Features DataFrame dtypes:\n{self.user_features.dtypes}")
        logger.info(f"User Features DataFrame sample (first 3 rows):\n{self.user_features.head(3).to_string()}")
        
        logger.info(f"\nItem Features DataFrame shape: {self.item_features.shape}")
        logger.info(f"Item Features DataFrame columns: {list(self.item_features.columns)}")
        logger.info(f"Item Features DataFrame dtypes:\n{self.item_features.dtypes}")
        logger.info(f"Item Features DataFrame sample (first 3 rows):\n{self.item_features.head(3).to_string()}")
        
        logger.info("="*80)
        logger.info("END OF DATASET INITIALIZATION - DETAILED INFORMATION")
        logger.info("="*80 + "\n")
