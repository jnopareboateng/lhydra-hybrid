...
def main():
    ...
    try:
        ...
        if args.debug:
            logger.debug(f"User features columns: {user_features_df.columns.tolist()}")
            logger.debug(f"Item features columns: {item_features_df.columns.tolist()}")
    ...
    try:
        # Check for feature manifest path
        manifest_path = args.manifest
        if manifest_path is None and 'feature_manifest_path' in config.get('data', {}):
            manifest_path = config['data']['feature_manifest_path']
            logger.info(f"Using feature manifest from config: {manifest_path}")
