...
class TagBasedAudioFeaturePredictor:
    ...
    def _preprocess_tags(self, tags_series):
        ...
        for tags in tqdm(tags_series, desc="Preprocessing tags", unit="track"):
            if pd.isna(tags):
    ...
    def _prepare_data(self, df):
        ...
        logger.info("Preparing data and extracting features...")
        ...
        if self.include_metadata:
            ...
            if 'name' in df.columns:
                ...
                logger.info("Processing track titles from Artist and Title...")
                df['processed_name'] = df['Artist and Title'].apply(
                    lambda x: self._preprocess_tags([x.split(' - ')[1] if ' - ' in str(x) else ''])[0]
                )
        
        logger.info("Adding handcrafted features based on genre keywords...")
        # Add handcrafted features based on genre keywords
        for idx, feature in tqdm(enumerate(['has_dance_related', 'has_acoustic_related', 'has_live_related', 'has_rock_related', 'has_rap_related']), 
                               desc="Creating handcrafted features", total=5):
            if idx == 0:
    ...
    def _download_glove_embeddings(self):
        ...
        try:
            total_lines = sum(1 for _ in open(glove_path, 'r', encoding='utf-8'))
            with open(glove_path, 'r', encoding='utf-8') as f:
                for line in tqdm(f, total=total_lines, desc="Loading word embeddings", unit="vectors"):
                    values = line.split()
                    word = values[0]
    ...
    def _vectorize_data(self, df):
        ...
        if self.use_pretrained and self.embedding_model:
            ...
            logger.info("Converting text to embeddings...")
            tag_features = np.array([self._get_text_embedding(text) for text in tqdm(df['processed_tags'], desc="Vectorizing tags", unit="track")])
            ...
            if self.include_metadata:
                ...
                # Ensure consistent feature generation by checking for required columns
                required_columns = ['processed_artist', 'processed_name']
                for col in required_columns:
                    if col not in df.columns:
                        logger.warning(f"Missing {col} column, adding zero vectors")
                        df[col] = ""
                
                # Get artist embeddings
                artist_features = np.array([self._get_text_embedding(text) for text in tqdm(df['processed_artist'], desc="Vectorizing artists", unit="track")])
                ...
                # Get name embeddings
                name_features = np.array([self._get_text_embedding(text) for text in tqdm(df['processed_name'], desc="Vectorizing track names", unit="track")])
                ...
                # Ensure handcrafted features exist
                ...
                for col in handcrafted_cols:
                    if col not in df.columns:
                        logger.warning(f"Missing {col} column, adding zero values")
                        df[col] = 0
                ...
                logger.info("Combining all feature vectors...")
        ...
        # Fallback to TF-IDF if pretrained embeddings not available
            ...
            if not self.vectorizer:
                ...
                logger.info("Transforming data with TF-IDF vectorizer...")
                # Ensure we use the same feature set as during training
                if not self.feature_names:
                    logger.warning("No feature names available. Feature mismatch may occur.")
                
                # Transform using the pre-trained vectorizer
                tfidf_features = self.vectorizer.transform(df['processed_tags'])

                # The vectorizer might produce different features - ensure consistent shape
                tfidf_feature_count = tfidf_features.shape[1]
                expected_feature_count = len(self.feature_names) - 5  # Subtract handcrafted features
                
                if tfidf_feature_count != expected_feature_count:
                    logger.warning(f"TF-IDF feature count mismatch: got {tfidf_feature_count}, expected {expected_feature_count}")
                    
                    # Create a matrix of zeros with the expected feature count
                    logger.info("Creating feature matrix with the correct shape...")
                    tfidf_features_fixed = np.zeros((df.shape[0], expected_feature_count))
                    tfidf_features = tfidf_features_fixed
                else:
    ...
    def fit(self, train_df):
        ...
        # Progress bar for different model groups
        model_groups = [
            ("musical", musical_regressor := MultiOutputRegressor(musical_model), musical_indices),
            ("rhythm", rhythm_regressor := MultiOutputRegressor(rhythm_model), rhythm_indices),
            ("technical", technical_regressor := MultiOutputRegressor(technical_model), technical_indices),
            ("categorical", categorical_regressor := MultiOutputRegressor(categorical_model), categorical_indices)
        ]
        
        for model_name, regressor, indices in tqdm(model_groups, desc="Training model groups", total=len(model_groups)):
            logger.info(f"Training models for {model_name} features...")
            regressor.fit(X, y[:, indices])
            self.model[model_name] = regressor
    ...
    def evaluate(self, test_df):
        ...
        for model_type, indices in tqdm(self.feature_indices.items(), desc="Predicting with specialized models"):
        ...
        # Calculate metrics
        results = {}
        for i, feature in enumerate(tqdm(self.AUDIO_FEATURES, desc="Calculating metrics")):
    ...
    def _analyze_feature_importance(self):
        ...
        # Iterate through model types with progress bar
        for model_type, indices in tqdm(self.feature_indices.items(), desc="Analyzing model types"):
            model = self.model[model_type]
            
            # Process each target feature with progress bar
            for i, target_idx in enumerate(tqdm(indices, desc=f"Processing {model_type} features")):
    ...
    def _plot_feature_importance(self, importance_by_target, targets=None, top_n=15):
        ...
        for i, target in enumerate(tqdm(targets, desc="Plotting feature importance")):
    ...
    def predict(self, df):
        ...
        # Make predictions using each specialized model
        for model_type, indices in tqdm(self.feature_indices.items(), desc="Predicting with model groups"):
...
def main():
    ...
    try:
        logger.info("Loading Spotify data for prediction...")
